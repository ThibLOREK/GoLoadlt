...
func Catalogue() []map[string]any {
	items := []map[string]any{
		...
		meta("transform.fillna", "transform", "Fill NA", "Remplace les valeurs nulles — parité complète df.fillna (value, method ffill/bfill, columns, limit)", 1, 1, 1, 1),
		meta("transform.rename", "transform", "Rename Columns", "Renomme des colonnes — parité complète df.rename (columns mapping ancien:nouveau, errors ignore/raise)", 1, 1, 1, 1),
		meta("transform.drop_duplicates", "transform", "Drop Duplicates", "Supprime les doublons — parité complète df.drop_duplicates (subset, keep first/last/false, ignore_index)", 1, 1, 1, 1),
